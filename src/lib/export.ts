import { buildMosaic, type DensityMaps } from '@/engine/mosaic'
...
export async function runExport(
  ...
  const result = buildMosaic(ctx.maps, DEFAULT_SET, ctx.palette, {
    ...ctx.options,
    width: plan.outWidth,
  })
  ...
  if (plan.tiles === 1) {
    onProgress?.(0, 1)
    if (req.format === 'png') {
      download(await renderPNG(result, req.background), `${stamp}.png`)
    } else {
      const svg = toSVG(result, DEFAULT_SET, { background: req.background })
  ...
  for (let ty = 0; ty < plan.rows; ty++) {
    for (let tx = 0; tx < plan.cols; tx++) {
      ...
      if (req.format === 'png') {
        const blob = await renderPNG(result, req.background, [x, y, w, h])
        entries.push({ name: `${name}.png`, data: new Uint8Array(await blob.arrayBuffer()) })
      } else {
        const svg = toSVG(result, DEFAULT_SET, {
...
async function renderPNG(
  ...
  drawMosaic(ctx, result, DEFAULT_SET, {
    background,
    origin: { x: cx, y: cy },
